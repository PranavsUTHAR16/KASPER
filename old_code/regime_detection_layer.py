...
class RegimeDetectionLayer(nn.Module):
    ...
    def forward(self, phi_t, tau=1.0, hard=False, threshold=None):
        # Deterministic forward pass: Spline -> Classifier -> Regime Projection Logits
        z_spline = self.spline(phi_t, threshold=threshold)
        z = self.classifier_block(z_spline)
        logits = self.regime_projection(z)
        
        if self.training:
            # Eq 17: Gumbel-Softmax on logits to sample regimes stochastically
            # Generate Gumbel noise: g = -log(-log(U))
            U = torch.rand_like(logits)
            g = -torch.log(-torch.log(U + 1e-20) + 1e-20)
            
            # Add noise and scale by temperature tau
            probabilities = F.softmax((logits + g) / tau, dim=-1)
            
            if hard:
                # Straight-Through Estimator for one-hot routing
