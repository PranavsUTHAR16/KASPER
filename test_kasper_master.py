...
def test_master_pipeline():
    ...
    # 3. Instantiate KasperCompositeLoss
    print("\nInstantiating KasperCompositeLoss...")
    composite_loss_fn = KasperCompositeLoss(
        lambda_s=1e-3,
        lambda_c=0.01,
        lambda_o=0.01,
        lambda_b=0.05
    ...
    print(f" - Predictions shape: {predictions.shape} (Expected: [{batch_size}, 1])")
    ...
    # Checks
    assert predictions.shape == (batch_size, 1), f"Unexpected predictions shape: {predictions.shape}"
